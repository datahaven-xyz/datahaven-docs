...
async function run() {
  ...
  // --- Data deleting logic ---
  // **PLACEHOLDER FOR STEP 4: CHECK PENDING DELETION COUNT (BEFORE)**
  // **PLACEHOLDER FOR STEP 5: REQUEST FILE DELETION**
  // **PLACEHOLDER FOR STEP 6: CHECK PENDING DELETION COUNT (AFTER)**
  // **PLACEHOLDER FOR STEP 7: WAIT FOR BACKEND TO RETURN EMPTY BUCKET**
  // **PLACEHOLDER FOR STEP 8: DELETE A BUCKET**
...
3. Add the following code:
    ...
    --8<-- 'code/store-and-retrieve-data/use-storagehub-sdk/manage-files-and-buckets-sc/bucketOperations.ts:imports'

    --8<-- 'code/store-and-retrieve-data/use-storagehub-sdk/manage-files-and-buckets-sc/bucketOperations.ts:get-buckets-msp'
...
--8<-- 'code/store-and-retrieve-data/use-storagehub-sdk/manage-files-and-buckets-sc/fileOperations.ts:imports'

--8<-- 'code/store-and-retrieve-data/use-storagehub-sdk/manage-files-and-buckets-sc/fileOperations.ts:get-bucket-files-msp'
...
## Check Pending Deletion Requests (Before)

Before requesting a file deletion, you can query the FileSystem Precompile to check how many pending file deletion requests currently exist for your account. This provides a useful before-and-after comparison to confirm that your deletion request was registered.

### Add Method to Get Pending Deletion Count

Add the following code to your `fileOperations.ts` file:

```ts title="fileOperations.ts"
--8<-- 'code/store-and-retrieve-data/use-storagehub-sdk/manage-files-and-buckets-sc/fileOperations.ts:get-pending-file-deletion-requests-count'
```

### Call the Pending Deletion Count Helper Method
...
To request file deletion, create a helper method called `requestDeleteFile` in your `fileOperations.ts` file and then update the `index.ts` file accordingly, to execute that logic. First, fetch the file's metadata from the MSP and then submit a deletion request by calling the FileSystem Precompile directly via `walletClient.writeContract`.

Unlike the SDK path where file deletion is a single method call, the smart contract path requires you to manually construct and sign the deletion intention. This involves building a 33-byte raw message (32-byte file key + 1-byte operation code), signing it with EIP-191 `personal_sign`, and passing both the signed intention and signature to the precompile.
...
```ts title="fileOperations.ts"
--8<-- 'code/store-and-retrieve-data/use-storagehub-sdk/manage-files-and-buckets-sc/fileOperations.ts:request-file-deletion'
```
...
## Check Pending Deletion Requests (After)
...
1. Add the following code in your `bucketOperations.ts` file:
    ...
    ```ts title="bucketOperations.ts"
    --8<-- 'code/store-and-retrieve-data/use-storagehub-sdk/manage-files-and-buckets-sc/bucketOperations.ts:wait-for-backend-bucket-empty'
    ```
...
??? code "View complete `bucketOperations.ts`"
    ...
    ```ts title="bucketOperations.ts"
    --8<-- 'code/store-and-retrieve-data/use-storagehub-sdk/manage-files-and-buckets-sc/bucketOperations.ts'
    ```

??? code "View complete `fileOperations.ts`"

    ```ts title="fileOperations.ts"
    --8<-- 'code/store-and-retrieve-data/use-storagehub-sdk/manage-files-and-buckets-sc/fileOperations.ts'
    ```
